...
const MC_HUD_BASE = 'https://raw.githubusercontent.com/PrismarineJS/minecraft-assets/master/data/1.21.8/gui/sprites/hud/';
const ICON_URLS = {
  ...
  cart:    MC_ASSET_BASE + 'tnt_minecart.png',
  mace:    MC_ASSET_BASE + 'mace.png',
  nethpot: MC_ASSET_BASE + 'netherite_helmet.png',
  diapot:  MC_ASSET_BASE + 'potion.png',
  uhc:     MC_HUD_BASE + 'heart/hardcore_full.png',
};
...
function titleFor(points, rank){
  if(rank === 1) return LEGEND_TITLE;
...
function renderPanel(){
  ...
  const list = getFilteredPlayers().map(p => ({ p, pts: overallPoints(p) }))
...
function openProfileModal(id){
  ...
  const title = titleFor(pts, overallRank);

  const positions = [
    { rank: overallRank, label: 'OVERALL', sub: `${pts} points`, iconName: 'overall' },
  ];
...
function renderInfoBody(){
  ...
  const legendRowHtml = `
    <div class="ct-titles-row">
      <span class="nm ct-ptitle ${LEGEND_TITLE.color}">${LEGEND_TITLE.label}</span>
      <span class="rng">#1 overall</span>
    </div>
  `;
  ...
  document.getElementById('ctInfoBody').innerHTML = `
    ...
    <div>
      ...
      <div class="ct-titles-list">${legendRowHtml}${titlesHtml}</div>
    </div>
  `;
}
